...
export function FavoritesProvider({children}) {
  const {session, authClient, login, ready, returnIntent, setReturnIntent} = useGalleryAuth();
  ...
  useEffect(() => {
    ...
    setLoadedUser(null); setIds([]); setPending([]); setError(''); locks.current.clear();
    ...
    fetch('/api/favorites',{headers:{Authorization:`Bearer ${session.access_token}`}})
      ...
      .then(data => {if (current === generation.current) {setIds(data.caseIds); setLoadedUser(session.user.id);}})
      .catch(() => {if (current === generation.current) setError('load');})
      .finally(()=>{if(current === generation.current) setLoading(false);});
  }, [session?.user?.id]);
  useEffect(() => {
    if (!session || loadedUser !== session.user.id || loading || error || !returnIntent) return;
    if (returnIntent.type === 'favorites' || returnIntent.type === 'favorite') setOnlyFavorites(true);
    if (returnIntent.type === 'favorite' && Number.isSafeInteger(returnIntent.caseId) && !ids.includes(returnIntent.caseId)) toggle(returnIntent.caseId);
    setReturnIntent(null);
  }, [session, loadedUser, loading, error, returnIntent]);
  async function toggle(id) {
    if (!session) { login({type:"favorite", caseId:id}); return; }
  ...
  return <Favorites.Provider value={{ids,pending,loading:loading || !ready,error,onlyFavorites,setOnlyFavorites,toggle}}>{children}</Favorites.Provider>;
}
