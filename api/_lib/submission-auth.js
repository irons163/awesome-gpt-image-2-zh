...
export function submissionAuthConfig() {
  return { url: process.env.SUBMISSION_SUPABASE_URL, publishableKey: process.env.SUBMISSION_SUPABASE_PUBLISHABLE_KEY };
}
...
export async function submissionUser(req) {
  ...
  if (error || !data?.user?.email_confirmed_at || !data.user.app_metadata?.providers?.includes('google')) return null;
  return data.user;
}
