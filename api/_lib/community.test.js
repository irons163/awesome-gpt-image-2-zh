...
test('admin operations require super_admin and same-origin writes', () => {
  ...
  process.env.APP_URL = 'https://gpt-image.zero2codex.dev';
  try {
    assert.equal(validateSameOrigin({ headers: { origin: 'https://gpt-image.zero2codex.dev', host: 'gpt-image.zero2codex.dev' } }), true);
    assert.equal(validateSameOrigin({ headers: { origin: 'https://evil.example', host: 'gpt-image.zero2codex.dev' } }), false);
  } finally {
